...
describe('Nitro', () => {
	...
	it('Will throw an error an invalid child is given', () => {
		...
		comp.setInput(true);
		expect(() => comp.getElement()).toThrow(new Error('Cannot treat value as child: true; must be one of: string, HTMLElement, Component, null, or false.'));

		comp.setInput({});
		expect(() => comp.getElement()).toThrow(new Error('Cannot treat value as child: [object Object]; must be one of: string, HTMLElement, Component, null, or false.'));

		comp.setInput(9001);
		expect(() => comp.getElement()).toThrow(new Error('Cannot treat value as child: 9001; must be one of: string, HTMLElement, Component, null, or false.'));
	});
